...
RUN DEBIAN_FRONTEND=noninteractive bash -c 'yes | unminimize' && apt update && ln -fs /usr/share/zoneinfo/Asia/Shanghai /etc/localtime \
    && apt -y install asciidoc && dpkg-reconfigure --frontend noninteractive tzdata \
    && apt -y install antlr3 asciidoc autoconf automake autopoint binutils \
...
RUN DEBIAN_FRONTEND=noninteractive apt -y install android-sdk-ext4-utils iputils-ping openssh-server
